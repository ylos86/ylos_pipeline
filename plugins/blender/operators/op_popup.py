# -*- coding: utf-8 -*-
# Main popup opened from the header button.
...
class YLOS_OT_OpenPopup(bpy.types.Operator):
    ...
    def _draw_state_header(self, layout, context):
        ...
        if scene.ylos_current_asset:
            ...
            step.label(
                text=_step_full_label(scene.ylos_context_type, scene.ylos_current_step),
                icon="SEQUENCE",
            )
        else:
    ...
    def _draw_scene(self, layout, context):
        ...
        if not results:
            layout.separator(factor=0.3)
            hint = layout.box()
            hint.label(text="Scan the scene to check naming and readiness.", icon="INFO")
            return
