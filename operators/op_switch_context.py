# -*- coding: utf-8 -*-
# Ylos Pipeline - operators/op_switch_context.py
# Confirmation dialog before switching asset or step.
# Checks bpy.data.is_dirty to warn about unsaved changes.
...
class YLOS_OT_SwitchAsset(bpy.types.Operator):
    ...
    new_asset: StringProperty(
        name="Asset Name",
        description="Name of the asset to switch to",
        default="",
    )
    ...
    def draw(self, context):
        ...
        col.label(
            text=f"Switch to : {self.new_asset}",
            icon="FORWARD",
        )
        col.separator()
...
class YLOS_OT_SwitchStep(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        # Pre-fill with current step
        self.new_step = scene.ylos_current_step
    ...
    def draw(self, context):
        ...
        col.separator()
        col.label(
            text=f"Asset : {scene.ylos_current_asset}",
            icon="OBJECT_DATA",
        )
        col.label(
            text=f"Current step : {scene.ylos_current_step}",
            icon="FILE_BLEND",
        )
        col.separator()
